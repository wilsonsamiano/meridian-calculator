...
function emptyWorkspace() {
  return mkdtempSync(join(tmpdir(), "grok-og-empty-"));
}
...
test("platform chrome overwrites share-card metas and always sets og:title", () => {
  ...
  const out = injectGrokPwaHead(html, { appName: "Wild Race", cwd: emptyWorkspace() });
...
test("streaming injector matches </HEAD> case-insensitively", () => {
  const injector = createHeadInjector({ appName: "Wild Race", cwd: emptyWorkspace() });
  const chunks = [
